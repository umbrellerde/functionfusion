...
folder_prefix="$test_name-waaylater"
sizes=("128" "128" "128" "128") # TODO this needs to change??
##### Variables
...
sleep 12
terraform apply -auto-approve -var use_case="${use_case}" -var memory_sizes="[128,1024,1536]" ## TODO only go up to the wrong s3 bucket here to speed some things up?
...
makecold(){
    #aws lambda invoke --function-name coldstarts --cli-binary-format raw-in-base64-out /dev/null
...
for ((iteration=0; iteration<default_iterations; iteration++)) do
    ...
    for ((coldstart=0; coldstart<number_coldstarts; coldstart++)) do
        ...
        curl -X POST "$base_url/SYNC-A-${sizes[iteration]}" -H 'Content-Type: application/json' -d '{"test":"event"}'
        makecold
    done
