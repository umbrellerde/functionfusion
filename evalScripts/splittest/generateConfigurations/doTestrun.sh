...
##### Variables
# How many iterations?
default_iterations=12 # 3 calls must be set to local (A-B, B-D, D-E), then two more to test nondefault configurations, then one where all the best options are used
# How long per iteration? In seconds
duration=100 # 100
# Terraform Use Case
use_case="useCases/splittest"
# Requests per Second
...
for ((iteration=0; iteration<default_iterations; iteration++)) do
    ...
    k6 run -e BASE_URL="$base_url" -e RPS="$req_s" -e DURATION="$duration" "k6/$iteration.js"
...
# TODO extract 1 needs no extract afterwards....
