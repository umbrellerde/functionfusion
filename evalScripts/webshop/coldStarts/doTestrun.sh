...
##### Variables
number_coldstarts=60 # Times three because we send a request to multiple functions for that
# Terraform Use Case
use_case="useCases/webshop"
# Test name to identify different attempts
test_name="oldDistJun"
# Configuration File
...
folder_prefix="$test_name-$entry_task"
##### Variables
...
for ((iteration=0; iteration<default_iterations; iteration++)) do
    ...
    for ((coldstart=0; coldstart<number_coldstarts; coldstart++)) do
        ...
        curl -X POST "$base_url/SYNC-addcartitem" -H 'Content-Type: application/json' -d '{"userId":"USERCOLD","itemId": 1,"quantity": 1}'
        makecold
        curl -X POST "$base_url/SYNC-frontend" -H 'Content-Type: application/json' -d '{"userId":"USERCOLD","operation": "get", "currency": "USD"}'
        makecold
        curl -X POST "$base_url/SYNC-frontend" -H 'Content-Type: application/json' -d '{"userId":"USERCOLD","operation": "checkout", "currency": "USD"}'
        makecold
    done
